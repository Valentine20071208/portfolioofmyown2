...
const articleImageLayout = {
  "rhodes-in-the-elevator": [
    { number: "01", slot: 0, shift: "right" },
    { number: "02", slot: 1 },
    { number: "03", slot: 3 },
    { number: "04", slot: 4 }
  ],
  "meaning-in-the-forest": [
    { number: "01", slot: 0, shift: "right" },
    { number: "02", slot: 2 },
    { number: "03", slot: 3, shift: "right" }
  ],
  "spark-before-ash": [
    { number: "01", slot: 0, shift: "right" },
    { number: "02", slot: 2 }
  ],
  "greenhouse-and-railway": [
    { number: "01", slot: 0, shift: "right" },
    { number: "02", slot: 1 },
    { number: "03", slot: 2 }
  ]
...
function articleImages(article) {
  return (articleImageLayout[article.slug] || []).map((item, index) => {
    const src = `./assets/article-illustrations/${article.slug}/${article.slug}-${item.number}.png`;
    return { ...item, src, index };
  });
}

function articleImageHtml(article, image) {
  const { src, index, shift } = image;
  const side = index % 2 === 0 ? "right" : "left";
  const shiftClass = shift === "right" ? " article-float-shift-right" : "";
  return `
    <figure class="article-float article-float-${side}${shiftClass}" style="shape-outside: url('${src}');">
...
function renderArticle(article) {
  ...
  const imageSlots = images.reduce((slots, image) => {
    const slot = Math.min(article.content.length - 1, image.slot);
    slots[slot] = [...(slots[slot] || []), image];
    return slots;
  }, {});
  const paragraphs = article.content
    .map((text, index) => {
      const floats = (imageSlots[index] || [])
        .map((item) => articleImageHtml(article, item))
        .join("");
